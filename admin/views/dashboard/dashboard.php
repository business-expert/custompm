...
<div class="row-fluid sortable ui-sortable">
  ...
  <div class="box span4" style="width:45%">
    <div data-original-title="" class="box-header well">
      <h2><i class="icon-user"></i> <?=$lang['Internal Message Board']?></h2>
      <div class="box-icon"> <a class="btn btn-minimize btn-round" href="#"><i class="icon-chevron-up"></i></a></div>
    </div>
    <div class="box-content">
    	<?=$objHTML->getWorkGroupCombo('sr_WorkgroupID', $_REQUEST['sr_WorkgroupID'], 'style="width:100%" onChange="getDataBasedWorkGroup(this.value);"');?>
...
<div class="row-fluid sortable ui-sortable">
  ...
  <div class="box span4" style="width:45%">
    <div data-original-title="" class="box-header well">
      <h2><i class="icon-folder-open"></i> <?=$lang['Internal Documents']?></h2>
      <div class="box-icon"> <a class="btn btn-minimize btn-round" href="#"><i class="icon-chevron-up"></i></a> </div>
    </div>
    <div class="box-content">
        <?=$objHTML->getWorkGroupCombo('sr_WorkgroupID1', $_REQUEST['sr_WorkgroupID1'], 'style="width:100%" onChange="getWorkGroupDocument(this.value);"');?>
		<spna id='div_internal_doc'><?=$InternalDocuments?></span>
    </div>
  </div>
  </div>
<div class="row-fluid sortable ui-sortable">  
...
function getMessageModel(MessageID)
{
	if(MessageID > 0)
	{
		$.ajax({
				type	: "GET",
				url		: "ajax.php",
				data    : "ajaxcall=true&mod=getMessageModel&message_id="+MessageID+"&type=",
...
function getAgendaModel(AgendaID)
{
	if(AgendaID > 0)
	{
		$.ajax({
				...
				url		: "ajax.php",
				beforeSend: function () {},
				data    : "ajaxcall=true&mod=getAgendaModel1&agenda_id="+AgendaID,
				success	: function(result) 
						  { 
						  	data = JSON.parse(result);
...
{
	var WorkGroupID = $("#sr_WorkgroupID").val();
...
function getWorkGroupDocument(elmVal)
{
	$.ajax({
			type	: "GET",
			url		: "ajax.php",
			beforeSend: function () {},
			data    : "ajaxcall=true&mod=getWorkGroupDocument&workgroup_id="+elmVal+"&type=int",
			success	: function(result) 
					  {
						$("#div_internal_doc").html(result);
