...
class dashboard
{
	...
	function getAllMessageBoard($workgroup = 0)
	{
		...
		$SQL = "SELECT *, (SELECT count(*) FROM Message as B WHERE B.SubjectID = MessageSubjectID) as totalmessage,
					(SELECT MessageText FROM Message as B 
							WHERE B.SubjectID = MessageSubjectID ORDER BY DateCreated DESC LIMIT 1)  as message, 
					(SELECT MessageID FROM Message as B 
						WHERE B.SubjectID = MessageSubjectID ORDER BY DateCreated DESC LIMIT 1)  as MessageID
	...
	function displayMessageBoard($record, $type = 'gen')
	{
		global $lang;
		...
		foreach($record as $key => $row)
		{
			...
			$arrMessageBoard[] = '<li>
									<div class="row-fluid">
									  ...
									  <div class="span2"><a href="javascript:getMessageModel(\''.$row->MessageSubjectID.'\',\''.$type.'\');">'.$lang['View'].'</a></div>
									</div>
								  </li>';
		}
