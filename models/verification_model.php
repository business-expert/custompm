...
class verification
{
	...
	function checkUniqUsername()
	{
		global $DB, $objComm, $lang;
		
		$SQL = "SELECT * FROM members WHERE username='".$_REQUEST['data_username']."'";
		$row = $DB->fetchOne($SQL);		

		if($row->id > 0)
		{
			$_SESSION['verification_error']  = 'username_exists';
			$objComm->redirect1("verify.php?model=verification&action=password&code=".$_REQUEST['code']);		
		}
	}
	...
	function createPassword()
	{
		...
		$SQL = "UPDATE members 
				   SET username='".$_REQUEST['data_username']."', `activation`= '' , `verified` = 'D', 
		...
		$errmsg = $lang["account_created_check_email"];
		include_once(VIEWS. "/error/activation.php");
		exit();
	}
	...
	public function sendmail()
	{
		...
		$arrData['{USERNAME}']   = $_REQUEST['data_username'];
